...
const Header = ({clicked}) => {
    ...
    return(
        <header  className="w-full z-10 bg-white select-none">
            <div className="container mx-auto py-3 px-2 flex flex-row justify-between items-center relative" >
                <div className="flex items-center">
                    <button onClick={clicked} className="md:hidden mr-6 text-3xl text-pri-dark focus:outline-none">
                ...
                <div className="h-full pl-5 md:mx-5 md:px-5 flex-1 flex items-center">
                    <div className="w-full bg-sec border-2 border-sec rounded flex relative">
                        <input onBlur={() => setInputValue(false)} onKeyPress={topSearchHandler} ref={inputRef} type="text" placeholder="Search for..." className="px-5 py-2 w-full focus:outline-none focus:bg-gray-50"/>
                        {
                            inputValue && <Portal selector="#modal"><TopSearch coords={coords} /></Portal>
                        }
                        <div className="flex justify-center items-center px-6">
                            <svg className="text-2xl text-white"  stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 24 24" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"></path></svg>
                        </div>
                    </div>
                </div>
                <div className="w-3/12 h-full hidden md:flex justify-between items-center icons">
                    <div className="p-2 relative rounded-full bg-white border border-gray-100 cursor-pointer text-pri hover:bg-orange-200 hover:bg-dark hover:bg-opacity-5">
                        {
                            cartOpen &&
                        ...
                        <span className="w-5 h-5 bg-gradient-to-r p-1 from-sec to-sec text-white absolute -top-3 text-xs rounded-full flex justify-center items-center  -right-2">{quantity}</span>
                    ...
                    <div  className="p-2 rounded-full bg-white text-pri border border-gray-100 cursor-pointer hover:bg-dark hover:bg-opacity-5">
